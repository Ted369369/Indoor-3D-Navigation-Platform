...
| `backend/` | `position_engine.py` (fusion service), `simulator.py` (hardware-free testing) |
...
Quick start: read [docs/SETUP.md](docs/SETUP.md). To try it with zero
hardware: configure HiveMQ in `backend/.env` + `web/config.js`, run
`python backend/position_engine.py` and `python backend/simulator.py`, then
serve `web/` locally.
